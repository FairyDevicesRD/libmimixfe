...
|timeToInactive_|800|ミリ秒|VAD によって、発話終了であると判定される最短の長さ。これが短い方が、より敏速に発話終了判定を得られるが、短すぎると発話途中で切られる場合がある|
...
XFEVadConfig クラスの上表の設定値は、全て 10 msec 以上である必要があります。かつ、tailPaddingTime_ は timeToInactive_ 以下である必要があります（tailPaddingTime_ <= timeToInactive）、上述の通り、timeToInactive は、発話終了であると判定される長さであり、tailPaddingTime_ は、発話終了時点から後方に与えられるマージンです。tailPaddingTime_ が timeToInactive_ より長いということはすなわち、発話終了と判定されるフレームよりも切り出される長さが長いということになり、これは次の発話が、現在の発話の出力区間に混じる可能性があるために認められません。
...
v1.0 から追加された recorderCallback の呼び出され方を制御する設定データクラス。デフォルトのまま利用することが推奨されます。outputType に allFrames を指定した場合、音声出力がない場合もコールバック関数が呼ばれます。これは、開発時などに、全時間に渡りストリーム情報を確認したい場合に有用です。
