...
|変数名|初期値|説明|
|---|---|---|
|samplingrate_|16000|libmimixfe の信号処理パイプラインに導入する音声のサンプリングレート。現在は 16000 以外対応していません。|
|microphoneUsage_[18]|ヘッダファイル参照|マイクの利用用途。どのマイクを録音用とし、どのマイクを参照用とし、どのマイクを単に無視するかを設定できる。これによって、少ないマイク数、例えば 4ch や 8ch のみを用いた試行を行うことができるが、現在は、初期値以外未対応|
