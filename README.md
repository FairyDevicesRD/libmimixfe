...
この関数は、`XFERecorder` クラスのデストラクタで呼び出されるので、典型的には、ユーザープログラム側から明示的に呼び出す必要はありません。Tumbler での録音を完全に停止したい場合、例えば、マイクミュートがユーザーから指示された場合、などのみに利用します。マイクミュートは、単にコールバック関数に与えられた音声データを無視するという方法によっても実現することができます。マイクミュート指示を、どのレベルで捉えるか、という設計指針の問題となるでしょう。
